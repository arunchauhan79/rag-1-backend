...
from schema import OrgOutput, OrgCreate, OrgModel
...
from core import UserAlreadyExistsException, DatabaseConnectionException,DatabaseQueryException,NotFoundException
import logging
...
async def createOrg(org_data: OrgCreate, db:AsyncDatabase) -> str:
    try:
        ...
        result = await db.organizations.insert_one(org_document)
        print("result",result)
...
async def get_organization_by_id(org_id: str, db:AsyncDatabase = Depends(get_database)) -> OrgOutput:
    try:
        if db is None:
